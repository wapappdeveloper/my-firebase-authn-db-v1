...
export class MainComponent implements OnInit {
  params: any = {
    actions: [
      ...
      { label: "profile", icon: "profile.png" },
      { label: "info", icon: "info.png" },
    ],
    header:'main'
  };
  constructor(private commonService:CommonService, private globalService:GlobalService, private datapersistanceService:DataPersistenceService, private databaseService:DatabaseService) { }
  ...
  emitter(obj: any) {
    if(obj && obj.event && obj.event==='action' && obj.action){
      switch(obj.action){
        ...
        case 'profile':this.commonService.navigateTo('profile');break;
